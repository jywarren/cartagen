/**
 * @namespace Namespace for methods to detect and handle partial coastlines in
 *            data.
 */
var Coastline = {
	/**
	 * Binds {@link Coastline.draw} to cartagen:predraw
	 */
	initialize: function() {
		$('canvas').observe('cartagen:predraw',
		                    Coastline.draw.bindAsEventListener(this))
	},
	/**
	 * Array of coastline ways to be assembled occasionally into coastline
	 * 'collected way' relations
	 */
	coastlines: [],
	/**
	...
	coastline_nodes: [],
	/**
	 * Array of current coastline nodex, as well as their entry and exit angle.
	 * Each member of this array is and array in the format
	 * [coastline_node_array, [entry_angle, exit_angle]]. Entry and exit angles
	 * are in radians, measured clockwise from the +y axis.
	 */
	assembled_coastline: [],
	/**
	 * Refreshes coastline data. Bound to cartagen:predraw
	 */
	draw: function() {
		Coastline.assembled_coastline = []
		Data.relations.values().each(function(object) {
			// invent a better way to trigger collect_nodes, based on Viewport
			// change:
			//
			// if (Glop.frame == 0 || Glop.frame % 30 == 0)
			//     object.collect_nodes()
			
			if (Config.coastline_debug) $l(this.id+' relation')
			
			object.collect_nodes()
			if (object.coastline_nodes.length > 0)
				Coastline.assembled_coastline.push([object.coastline_nodes,
				                                   [object.entry_angle,
												    object.exit_angle]])
		})
		...
		if (Coastline.assembled_coastline.length > 0) {
			...
			Coastline.assembled_coastline.each(function(coastline,index) {
				coastline.push(Viewport.nearest_corner(coastline[0].first()[0],
				                                       coastline[0].first()[1]))
													   
				coastline.push(Viewport.nearest_corner(coastline[0].last()[0],
				                                       coastline[0].last()[1]))
			})
			...
			Coastline.assembled_coastline.each(function(coastline,index) { 
				...
				if (Config.coastline_debug) {
					$l(coastline[0].length + ':'+coastline[2][2]+'=start ' + 
					   coastline[0].first()[0] + ','+coastline[0].first()[1] + 
					   '=first')
				...
				coastline[0].each(function(node,c_index) { 
					...
					if (Config.coastline_debug) {
						...
						$C.draw_text("Helvetica", 20, 'white', node[0]-10, 
						             node[1]-10, 
									 parseInt((coastline[1][0]*180)/Math.PI) + 
									           ',' + index + ':'+c_index)
						$C.save()
					}
				}) 
				...
				if (Coastline.assembled_coastline[index+1]) {
					...
					if (index != Coastline.assembled_coastline.length-1) {
						...
						if(Config.coastline_debug) {
							$l('walking to beginning!: '+
						       coastline[3][2] + "/" + coastline[2][2] + ':' + 
							    Coastline.walk(coastline[3][2],
							                   coastline[2][2],false).inspect())
						}
					...
					if (coastline[2][2] !=Coastline.assembled_coastline[index+1][2][2]) {
						...
						if(Config.coastline_debug) $l('walking to next!: '+coastline[2][2]+"/"+Coastline.assembled_coastline[index+1][2][2]+':'+Coastline.walk(coastline[2][2],Coastline.assembled_coastline[index+1][2][2]).inspect())
			...
			if ((end_corner[2] == start_corner[2]) && (end_angle < start_angle)) {
				if(Config.coastline_debug) $l('no-walk!!')
			} else if (end_corner[2] != start_corner[2] || end_angle > start_angle) {
				corners.push(end_corner[2],start_corner[2])
				if(Config.coastline_debug) $l('walking around!: '+end_corner[2]+"/"+start_corner[2]+':'+Coastline.walk(end_corner[2],start_corner[2]).inspect())
				Coastline.walk(end_corner[2],start_corner[2]).each(function(n) {
					$C.line_to(n[0],n[1])
				})
			}
			if(Config.coastline_debug) $l('ending: '+corners)
	...
	/**
	 * jeff's debugging method to do something with the something that has
	 * something to do with something. It was used at some time to do something
	 * somewhere in the code, and someone probably knows something about what it
	 * does with something.
	 * @ignore
	 */
